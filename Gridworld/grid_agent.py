...
#TODO: Finish off the replay buffer update for agent end
...
def agent_init(random_seed):
    ...
    if AGENT == TABULAR:
        ...
        #Context is a sliding window of the previous n states that gets added to the replay buffer for use by the auxiliary prediction tasks
        cur_context = []
...
def update_replay_buffer(cur_state, reward, next_state):
    global cur_context, zero_reward_buffer, non_zero_reward_buffer
    """
    ...
    if len(cur_context) == N:
        ...
        cur_transition.states = cur_context
        cur_transition.reward = reward
        cur_transition.next_state = next_state
        cur_context.pop(0)

    #TODO: Look into capping the buffer: probably need to do this so that we are not sampling from old biased experiences due to sampling error (in stochastic environments)
    if cur_transition:
        if reward == 0:
            zero_reward_buffer.append(cur_transition)
        else:
            non_zero_reward_buffer.append(cur_transition)
#def update_network_weights(cur_state):
    #"Update the neural network's weights given the current state"
